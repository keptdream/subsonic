...
mkdir -p ${SUBSONIC_HOME} ${SUBSONIC_DEFAULT_MUSIC_FOLDER} ${SUBSONIC_DEFAULT_PODCAST_FOLDER} ${SUBSONIC_DEFAULT_PLAYLIST_FOLDER}

# Remove database lock
if [ -f ${SUBSONIC_HOME}/db/subsonic.lck ]; then
    rm -f ${SUBSONIC_HOME}/db/subsonic.lck
fi

chown -R ${SUBSONIC_USER}:${SUBSONIC_USER} ${SUBSONIC_HOME} ${SUBSONIC_DEFAULT_MUSIC_FOLDER} ${SUBSONIC_DEFAULT_PODCAST_FOLDER} ${SUBSONIC_DEFAULT_PLAYLIST_FOLDER}
...
su - ${SUBSONIC_USER} -c  "cd ${SUBSONIC_ROOT} && ${JAVA} -Xmx${SUBSONIC_MAX_MEMORY}m -Dsubsonic.home=${SUBSONIC_HOME} -Dsubsonic.host=${SUBSONIC_HOST} -Dsubsonic.port=${SUBSONIC_PORT} -Dsubsonic.httpsPort=${SUBSONIC_HTTPS_PORT} -Dsubsonic.contextPath=${SUBSONIC_CONTEXT_PATH} -Dsubsonic.db="${SUBSONIC_DB}" -Dsubsonic.defaultMusicFolder=${SUBSONIC_DEFAULT_MUSIC_FOLDER} -Dsubsonic.defaultPodcastFolder=${SUBSONIC_DEFAULT_PODCAST_FOLDER} -Dsubsonic.defaultPlaylistFolder=${SUBSONIC_DEFAULT_PLAYLIST_FOLDER} -Djava.awt.headless=true -verbose:gc -jar subsonic-booter-jar-with-dependencies.jar"
